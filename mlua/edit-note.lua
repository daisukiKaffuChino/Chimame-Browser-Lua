...
function onCreate()
          ...
          id="ll",
          {
            ...
            };
            {
              ...
              onClick=function()
                ...
                跳转页面("mlua/code-play",{editor.Text})
              end;
              gravity="center";
              {
                ImageView;
  ...
  bardata={}
  baradp=LuaAdapter(activity,bardata,baritem)
  ps_bar.setAdapter(baradp)
  bardata={
    "FUN",
    "(",
    ")",
  ...
  for i,v in ipairs(bardata) do
    baradp.add({text={text=tostring(v),
  ...
  zdbti=Ticker()
  zdbti.Period=30000
  zdbti.onTick=function()
    写入文件(_link,editor.getText())
  end
  zdbti.start()
...
function onDestroy()
  写入文件(_link,editor.getText())
  zdbti.stop()
  activity.result({"mlua/edit-note"})
end
