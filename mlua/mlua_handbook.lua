...
function onCreate()
  ...
  --设置视图
  activity.setContentView(loadlayout("layout/bookmain"))
  ...
  drawer_lv.setOnItemClickListener(AdapterView.OnItemClickListener{
    onItemClick=function(id,v,zero,one)--侧滑
      ...
      if s=="退出" then
       ...
       elseif s=="关于" then
        activity.newActivity("mlua/about")
       elseif s=="设置" then
        activity.newActivity("mlua/settings")
       elseif s=="捐赠muk" then
        activity.newActivity("mlua/web",{"https://www.mukapp.top/pay/"})
       else
        Snakebar(s)
      end
    end})
...

    {
      ...
      onClick=function()end;
      {
        ...
        };
        {
          ...
          layout_height="-1";
          textColor="#00000000";
          onClick=function(v)跳转页面("mlua/web",{v.Text})end;
        };
      };
    };
  ...
  dmlist.setOnItemClickListener(AdapterView.OnItemClickListener{
    onItemClick=function(id,v,zero,one)
      ...
      跳转页面("mlua/view-code",{v.Tag._title.Text,v.Tag._text.Text})
    end})
  ...
  jclist.setOnItemClickListener(AdapterView.OnItemClickListener{
    onItemClick=function(id,v,zero,one)
      跳转页面("mlua/view-tutorial",{v.Tag._title.Text,one,jcdata})
    end})
  ...
  spjc.setOnItemClickListener(AdapterView.OnItemClickListener{
    onItemClick=function(id,v,zero,one)
      跳转页面("mlua/video-play",{v.Tag._text.Text})
  ...
  function 新建笔记()
    activity.newActivity("mlua/new-note")
  end
...
"全部笔记"页面可以添加/删除/重命名笔记
  ...
  notelist.setOnItemClickListener(AdapterView.OnItemClickListener{
    onItemClick=function(id,v,zero,one)
      跳转页面("mlua/edit-note",{v.Tag.text.Text})
    end})
  ...
  notelist.setOnItemLongClickListener(AdapterView.OnItemLongClickListener{
    onItemLongClick=function(id,v,zero,one)
      ...
      三按钮对话框(name,"请选择操作","重命名","删除","取消",function()
        跳转页面("mlua/new-note",{v.Tag.text.Text})
        关闭对话框()
      end,function()
        关闭对话框()
